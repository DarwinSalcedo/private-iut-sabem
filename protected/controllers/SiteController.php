		...
		if(isset($_POST['ContactForm']))
		{
			...
			if($model->validate())
			{
				...
				#mail(Yii::app()->params['adminEmail'],$subject,$model->body,$headers);
					...
					$model->subject,
					"CONTACTENOS ".$model->name." CORREO ".$model->email." ".$model->body);
	...
	public function actionLogin()
	{
		...
		if(isset($_POST['LoginForm']))
		{
			...
			if($model->validate() && $model->login()){
				...
				elseif (yii::app()->user->getState('acceso')=='SYSTEM') {
						Yii::app()->user->setFlash('error',' Señor usuario le notificamos que el acceso al sistema se encuentra bloqueado.');					
					}	
					else
		...
		if(isset($_POST['RegistroForm']))
		{
			...
			if(!$model->validate())
				...
				{				
								...
								$model->correo,
								"REGISTRO EXITOSO",
								"Señor usuario, $model->username gracias por registrarte  ");
			...
			if(!$model->validate())
				...
				{	
					...
					#$msg='La contraseña de '."$aux->username $aux->cedula".' ha sido reiniciada satisfactoriamente ';
						#var_dump($model->correo);
						#yii::app()->end();
					if($aux != null){
						if ($model->correo == $aux->correo) {
								...
								"Señor usuario su contraseña ha sido restablecida ");
							...
							Yii::app()->user->setFlash('error','Ups  asegura colocar tus credenciales correctamente');	
						}
					...
					$this->refresh();  
						...
						Yii::app()->user->setFlash('error','Ups  cedula  errada');
						$this->refresh(); 
					}
